...
class ManagerContactAdmin(admin.ModelAdmin):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                for row in csv.reader(io_string, delimiter=',', quotechar='"'):
                    ManagerContact.objects.update_or_create(
                        branch=row[0],
                        defaults={
                            'region': row[1],
                            'mobile_number': row[2],
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        return super(ManagerContactAdmin, self).changelist_view(request, extra_context=extra_context)
...
admin.site.register(ManagerContact, ManagerContactAdmin)